...
def main():
    ...
    all_probs = []
    all_true_labels = []
    aa_to_idx = {'A': 0, 'C': 1, 'D': 2, 'E': 3, 'F': 4, 'G': 5, 'H': 6, 'I': 7, 'K': 8, 'L': 9,
    ...
    for pdict, info in pred:
        ...
        original_aa = info['original']
        predicted_aa = max_key
        
        # Build sequences
        original_sequence += original_aa
        predicted_sequence += predicted_aa
        
        # Count accuracy
        if original_aa == predicted_aa:
            correct_predictions += 1
        total_residues += 1
        
        # Collect data for Top-3 accuracy
        if original_aa in aa_to_idx:
            true_idx = aa_to_idx[original_aa]
            prob_vector = [0.0] * 20
            for aa, prob in pdict.items():
                if aa in aa_to_idx:
                    prob_vector[aa_to_idx[aa]] = prob
            all_probs.append(prob_vector)
            all_true_labels.append(true_idx)
        
        # Display per-residue info
        print(' %4d %s %s:pred ' % (info['resnum'], original_aa, predicted_aa), end='')
        for aa in pdict.keys():
            print(' %5.3f:%s' % (pdict[aa], aa), end='')
        print('')

    # Calculate accuracy and Top-3
    accuracy = (correct_predictions / total_residues) * 100 if total_residues > 0 else 0
    
    try:
        top3_accuracy = top_k_accuracy_score(all_true_labels, all_probs, k=3, labels=list(range(20))) * 100
    except:
        top3_accuracy = 0.0

    # Display complete sequences
    print("\n" + "="*80)
    print("🧬 SEQUENCE COMPARISON")
    print("="*80)
    print(f"🎯 Top-3 Accuracy: {top3_accuracy:.1f}%")
    print(f"🔗 Protein Length: {total_residues} residues")
    
    print(f"\n🧬 Original Sequence:")
    print(f"   {original_sequence}")
    
    print(f"\n🎯 Predicted Sequence:")
    print(f"   {predicted_sequence}")
    
    # Color-coded comparison (showing differences)
    print(f"\n🔍 Sequence Alignment (mismatches marked with *):")
    alignment_line = ""
    for i, (orig, pred) in enumerate(zip(original_sequence, predicted_sequence)):
        if orig == pred:
            alignment_line += " "
        else:
            alignment_line += "*"
    ...
    if args.show_benchmark and live_metrics:
