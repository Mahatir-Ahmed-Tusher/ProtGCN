...
from protgcn.hypara import HyperParam, InputSource
from protgcn.dataset import BBGDataset
from protgcn.training import train, valid
from protgcn.models import ProtGCN, weights_init
...
if args.checkpoint_in != None:
    checkpoint = torch.load(args.checkpoint_in, weights_only=False)
    hypara = checkpoint['hyperparams']
    model = ProtGCN(hypara)
    params = model.size()
    ...
    ## Model Setup ##
    model = ProtGCN(hypara).to(source.device)
...
if source.onlypred is True:
    ...
    model = torch.load(source.param_in, map_location=torch.device(source.device), weights_only=False)
